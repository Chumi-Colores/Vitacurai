...
if __name__ == "__main__":
    ...
    try:
        # Procesar imágenes del directorio de calibración
        calib_data = extract_calibration_parameters(
        ...
        print(f"\nDatos de calibración extraídos:")
        print(f"  - {len(calib_data['objpoints'])} conjuntos de puntos 3D")
        print(f"  - {len(calib_data['imgpoints'])} conjuntos de puntos 2D")
        print(f"  - Imágenes exitosas: {calib_data['successful_images']}")
        print(f"  - Total de imágenes: {calib_data['total_images']}")
